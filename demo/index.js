...
const input = document.getElementById("formula-input"),
  ...
  layoutWaterfall = () => {
    ...
    cards.forEach((card) => {
      ...
      const rendered = card.querySelector(".rendered-math");
      if (rendered) {
        const math = rendered.querySelector("math");
        if (math) {
          const { style: math_style, scrollWidth: math_w } = math,
            { clientWidth } = rendered,
            max_w = clientWidth - 48;
          math_style.transform = "none";
          if (math_w > max_w) {
            math_style.transform = "scale(" + max_w / math_w + ")";
          }
        }
      }
  ...
  updateUI = () => {
    ...
    const cards = FORMULAS.map((formula, idx) => {
      ...
      render_box.innerHTML = mdMath(tex);
