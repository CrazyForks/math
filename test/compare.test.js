  ...
  files.forEach((file) => {
    if (file.endsWith(".yml") || file.endsWith(".yaml")) {
      const file_path = join(dir_path, file),
        ...
        cases = load(content) || [];
      cases.forEach(([md, expected], idx) => {
        test(file + " - Case " + idx, () => {
          const our_output = convert(md);
          expect(our_output.includes("\n")).toBe(false);
          expect(our_output.includes("\r")).toBe(false);
          ...
          expect(our_min).toBe(expected_min);
        });
      });
    }
  });
