  ...
  run = async () => {
    ...
    for (const [cat_name, data] of Object.entries(categories)) {
      ...
      for (const tex of data.katex) {
        ...
        try {
          const our_output = convert(input);
          ...
          if (our_min === expected_min) {
            ...
            WARN("Skip KaTeX unsupported convert formula: " + tex);
          }
        } catch (err) {
          WARN(
            "Skip KaTeX unsupported convert formula: " + tex + " (" + (err.message ?? err) + ")",
          );
        }
      }
      ...
      if (mathjax_promises_subset.length > 0) {
        ...
        outputs.forEach((output, idx) => {
          ...
          const { tex, input } = mathjax_candidates[idx];
          try {
            const our_output = convert(input);
            const our_min = normalize(minify(our_output, { collapseWhitespace: true }));
            const expected_min = normalize(minify(output, { collapseWhitespace: true }));
            if (our_min === expected_min) {
              ...
              WARN("Skip MathJax unsupported convert formula: " + tex);
            }
          } catch (err) {
            WARN(
              "Skip MathJax unsupported convert formula: " +
                tex +
                " (" +
                (err.message ?? err) +
                ")",
