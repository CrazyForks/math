...
Without heavy JS styling calculations, the compiler is incredibly fast:
...
- During Markdown parsing, `@webc.site/math` automatically catches syntax errors (like unmatched `\left(`,) and **gracefully falls back to displaying the raw TeX code**, eliminating the need for boilerplate `try...catch` blocks.
...
Use `@webc.site/math/md` to parse inline/block math formulas inside Markdown:
