...
由于避开了耗时的排版计算，它的编译速度达到了恐怖的级别（基于经典公式循环编译测试）：
...
`@webc.site/math` 的极小体积和高性能，得益于其高度聚焦的设计。它仅包含四个核心部分：
...
2. **Lexer**：高效切割 Token，只做基础的语法和符号分类。
3. **Parser**：将 Token 转为一颗扁平的抽象语法树（AST）节点。
4. **Renderer**：根据 AST 生成符合 MathML Core 规范的 XML 标签，如 `<mi>`（变量）、`<mo>`（运算符）、`<mn>`（数字）、`<mfrac>`（分式）等。

因为排版完全交给了浏览器，我们省去了所有位置计算、多语言字体打包和 DOM 拼接代码，从而用极少的代码实现了全套的核心逻辑。

### 强大的健壮性与容错

在前端，如果用户或 AI 输入了错误的 TeX 语法（例如没有闭合的 `\left(`），我们绝不希望页面因此崩溃。

- 在解析 Markdown 时，`@webc.site/math` 内部自动捕获编译阶段的语法错误，并**优雅退化为显示原始文本**，免去了手动写 `try...catch` 的繁琐。
...
可以使用 `@webc.site/math/md`，它会自动提取 Markdown 中的公式并直接转换成原生 MathML。
...
虽然现代浏览器能直接渲染 MathML，但为了获得像排版印刷品一样的精美效果，强烈建议配合专门的数学字体使用。我们推荐引用 `Latin Modern Math` 字体（源自高德纳经典的 Computer Modern）：
